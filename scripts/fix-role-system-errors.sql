-- Fix Role System Errors for LabSyncPro
-- This script fixes the type casting and policy dependency issues

-- 1. First, drop all policies that depend on the role column
...
-- 3. Add a temporary column for the new role type
ALTER TABLE public.users ADD COLUMN IF NOT EXISTS role_new user_role;

-- 4. Update the temporary column with converted values
UPDATE public.users 
SET role_new = CASE 
    WHEN role::text = 'super_admin' THEN 'super_admin'::user_role
    WHEN role::text = 'admin' THEN 'admin'::user_role
    WHEN role::text = 'lab_manager' THEN 'lab_manager'::user_role
    WHEN role::text = 'instructor' THEN 'instructor'::user_role
    WHEN role::text = 'lab_staff' THEN 'lab_staff'::user_role
...
-- 6. Set default value for role column
...
-- 10. Update the trigger function to handle role casting
...
BEGIN
    ...
    -- Determine user role with proper casting
    BEGIN
...
-- 13. Create test users with proper role casting
INSERT INTO public.users (
...
SELECT 'Role system errors fixed successfully!' as status;
