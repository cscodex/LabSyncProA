-- Complete Schema Reset for LabSyncPro
...
-- 17. Create function to create test admin user (will be called after auth.users exists)
...
BEGIN
    ...
    INSERT INTO public.users (
        ...
        admin_id, 'admin@labsyncpro.com', 'System', 'Administrator',
        'super_admin'::user_role, 'email', true, true, true, true
    );

    RETURN 'Test admin user created: admin@labsyncpro.com / LabSync2024!';
EXCEPTION WHEN OTHERS THEN
    RETURN 'Could not create test admin user: ' || SQLERRM;
END;
$$;

-- Note: Test users will be created via the trigger when they sign up normally
-- Or you can call: SELECT public.create_test_admin(); to create an admin user

-- 18. Create helper functions for user management
...
-- 20. Final status
SELECT 'Complete schema reset completed successfully!' as status,
       'Test users created for all roles' as test_users,
       'Triggers and RLS policies are active' as security,
       'Ready for Google OAuth testing' as oauth_status;
